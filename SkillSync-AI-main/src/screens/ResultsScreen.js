...
/**
 * Accepts results via route.params?.data or falls back to sample data.
 * data = { score, existing[], missing[], keywords[], resources[{title,platform,level,cost,url}] }
 */
...
const SAMPLE = {
  ...
  keywords: ["TypeScript", "Unit testing (Jest)", "React Router", "a11y", "CI/CD"],
  resources: [
    { platform: "FreeCodeCamp", title: "TypeScript for JS Devs", level: "Beginner", cost: "Free", url: "https://www.freecodecamp.org/" },
    { platform: "YouTube • Net Ninja", title: "Jest Crash Course", level: "Beginner", cost: "Free", url: "https://youtube.com" },
    { platform: "Udemy", title: "React Testing with Jest", level: "Intermediate", cost: "Paid", url: "https://udemy.com" },
    { platform: "Web.dev", title: "Accessibility Fundamentals", level: "Beginner", cost: "Free", url: "https://web.dev" },
  ],
};
...
export default function ResultsScreen({ route, navigation }) {
  ...
  useEffect(() => {
    Animated.timing(animated, { toValue: data.score, duration: 900, useNativeDriver: false }).start();
  }, [data.score, animated]);
    ...
    <LinearGradient colors={["#2AA6F2", "#173D87", "#C333B7"]} style={{ flex: 1 }}>
      ...
      >
        ...
        <Card title="Your Fit">
          ...
          >
            {/* simple ring look (no extra deps) */}
            <View
              style={{
                width: 108,
                height: 108,
            ...
            >
              <View
                style={{
                  ...
                  borderRadius: 54,
                  borderWidth: 10,
                  // color hint changes by score
                  borderColor: data.score >= 70 ? "#22C55E" : data.score >= 50 ? "#F59E0B" : "#EF4444",
              ...
              <Text style={{ fontSize: 22, fontWeight: "900", color: "#0f172a" }}>
                {/* animated number */}
                {/** Animated.Text can't interpolate to int easily; we round inside a listener-like approach: */}
                {Math.round(animated.__getValue?.() ?? 0)}%
              </Text>
        ...
        >
          <View style={{ gap: 10 }}>
            {data.resources.map((r, i) => (
              ...
              >
                <Text style={{ fontSize: 12, color: "#64748B" }}>{r.platform}</Text>
                <Text style={{ fontSize: 14, color: "#0F172A", fontWeight: "700" }}>{r.title}</Text>
                <Text style={{ fontSize: 12, color: "#64748B" }}>
                  {r.level} • {r.cost}
                </Text>
                <TouchableOpacity onPress={() => r.url && Linking.openURL(r.url)}>
                  <Text style={{ color: "#2563EB", marginTop: 6 }}>Open link →</Text>
                </TouchableOpacity>
              </View>
            ))}
          </View>
        ...
        <View style={{ flexDirection: "row", gap: 10, marginTop: 8 }}>
          ...
          <GhostBtn
            label="Save Result"
            onPress={() => {/* TODO: hook to backend / local save */}}
          />
        </View>
      </ScrollView>
    </LinearGradient>
  ...
  return (
    ...
    >
      <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 8 }}>
        ...
        {!!rightHint && <Text style={{ fontSize: 11, color: "#64748B" }}>{rightHint}</Text>}
      </View>
      {children}
    </View>
  );
  ...
  return (
    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
      {items.map((t, i) => (
        <View
          key={`${t}-${i}`}
          style={{ backgroundColor: color, paddingVertical: 6, paddingHorizontal: 10, borderRadius: 999 }}
        >
          <Text style={{ color: text, fontWeight: "700", fontSize: 12 }}>{t}</Text>
        </View>
      ))}
    </View>
  );
